		...
		if (is_array($passages))
			foreach ($passages as $passage) {
				...
				if ($passage != '') {
					...
					$parsed_start = parse_one (trim($startend[0]));
					...
					if ($parsed_start['chapter'] == 'wholebook') {
						...
						if ($count != 0 && $parsed_start['book'] == '' && $parsed_start['chapter'] != '') {
							$parsed_start['book'] = $parsed_return[$count-1]['end']['book'];
							if ($parsed_start['verse'] == '' && $parsed_return[$count-1]['end']['verse'] != '') {
								$parsed_start['verse'] = $parsed_start['chapter'];
								$parsed_start['chapter'] = $parsed_return[$count-1]['end']['chapter'];
							}
						}
						if (count($startend) === 1) {
							...
							$parsed_end = parse_one (trim($startend[1]), $parsed_start); // Verse range (e.g. Ex. 13:12-15)
					}
					if ($parsed_start['book'] == '')
						$parsed_return[$count] = array('raw' => $passage, 'start' => '', 'end' => '', 'error' => 1); // Cannot determine book. Return error.
					else {
						$single_chapter_books = array (31, 57, 63, 64);
						if ($parsed_start['verse'] == '')
						...
						if ($parsed_start['chapter'] < 1 | $parsed_start['verse'] < 1 | $parsed_end['chapter'] < 1 | $parsed_end['verse'] < 1)
							$parsed_return[$count] = array('raw' => $passage, 'start' => '', 'end' => '', 'error' => 2); // Cannot parse numerics. Return error.
						elseif ($parsed_end['bible'] < $parsed_start['bible'] | ($parsed_end['bible'] == $parsed_start['bible'] && $parsed_end['chapter'] < $parsed_start['chapter']) | (($parsed_end['bible'] == $parsed_start['bible'] && $parsed_end['chapter'] == $parsed_start['chapter'] && $parsed_end['verse'] < $parsed_start['verse'])))
							$parsed_return[$count] = array('raw' => $passage, 'start' => '', 'end' => '', 'error' => 3); // End before start. Return error.
						else
							$parsed_return[$count] = array('raw' => $passage, 'start' => $parsed_start, 'end' => $parsed_end, 'error' => 0); //Return parsed result.
					}
					$count++;
				}
			}
